...
class FEGAETaskConfig():
    def __init__(self, logger, modelName, showTrainingInfo=True, windowSize=100):
        self.logger = logger
    ...
    def getConfig(self, isCuda = False):
        ...
        if torch.cuda.is_available():
            ...
            errorModel.cuda()
            backwardModel.cuda()
        trainer = FEGAETrainer(forcastModel,backwardModel,errorModel, self.modelName, self.logger, 1e-3, self.showTrainningInfo, windowSize=self.windowSize)
        try:
            trainer.load()
        except:
            pass
